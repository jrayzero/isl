...
static int any_eliminated(isl_ast_build *build)
{
	...
	for (i = 0; i < build->depth; ++i)
		if (isl_ast_build_has_affine_value_for_elimination(build, i))
			return 1;
...
int isl_ast_build_has_affine_value_for_elimination(__isl_keep isl_ast_build *build,
	int pos)
{
	isl_aff *aff;
	int involves;
	...
	aff = isl_multi_aff_get_aff(build->values, pos);
	const char *n = isl_aff_get_dim_name(aff, isl_dim_in, pos);
	involves = isl_aff_involves_dims(aff, isl_dim_in, pos, 1);
	isl_aff_free(aff);
	if (n != NULL) {
	  if (n[0] == 'd' && n[strlen(n) - 1] == 'd') {
	    return 5;
	  }
	}
